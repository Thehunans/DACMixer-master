...
class DAFModel(nn.Module):
    ...
    def forward(self, x):
        ...
        mlp_att_out_2 = self.mlp_fusion_1(att_out_2)
        cross_att_out_1, cross_mod_out_1 = self.cross_att1(mlp_att_out_2, att_out_2)
        att_out_2 = self.da_att2(cross_att_out_1)
        mod_out_2 = self.mod4(cross_mod_out_1)
        # 第二次融合
        att_out_3, mod_out_3 = self.fusion_2(att_out_2, mod_out_2)
        mlp_att_out_3 = self.mlp_fusion_2(att_out_3)
        cross_att_out_2, cross_mod_out_2 = self.cross_att2(mlp_att_out_3, att_out_3)
        att_out = self.da_att3(cross_att_out_2)
        mod_out = self.mod5(cross_mod_out_2)
        # 第三次在融合
